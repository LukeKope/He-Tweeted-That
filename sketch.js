...
cb.setConsumerKey(
  "RRpHDAMRvaWyY5yEcqVvKDW9C",
  "AXcbrjlBtMxbrtqXkhAIKk36KTGlrHtlKcpRq12J7vbR7gs1i2"
);
...
function setup() {
  ...
  document.getElementById("left_tweet_p").innerHTML =
    "Welcome to He tweeted that?!";
  document.getElementById("right_tweet_p").innerHTML =
...
function checkVictoryCondition() {
  ...
  var left_card = document.getElementById("left_button"); // set var to be the element you want to listen for click on
  left_card.addEventListener("click", event => {
...
function updateTweets() {
  if (!(leftElementIsClicked == false && rightElementIsClicked == false)) {
    //If user clicked one of the answers
    //update the tweets
    ...
    if (card_decider % 2 == 0) {
      ...
      rightText = "Incorrect!";
      document.getElementById("left_tweet_p").innerHTML =
        real_acct_array[roundNumber]; //set tweets to cards
      document.getElementById("right_tweet_p").innerHTML =
        fake_acct_array[roundNumber];
    } else {
      rightText = "Correct!";
      leftText = "Incorrect!";
      document.getElementById("right_tweet_p").innerHTML =
...
function bonusRound() {
  let bonusTweetIndex = int(random(0, 7)); //select index for which to select for the bonus round
  if (!(leftElementIsClicked == false && rightElementIsClicked == false)) {
    //If user clicked one of the answers
    //update the tweets
    console.log("BONUS ROOOOOUND");
    leftText = "These are both real tweets!";
    rightText = "These are both real tweets!";
    document.getElementById("left_tweet_p").innerHTML =
      real_acct_array[roundNumber]; //set tweets to cards
    document.getElementById("right_tweet_p").innerHTML =
      bonusTweets[bonusTweetIndex]; //display the bonusTweet
  }
  ...
  if (roundNumber <= roundsCounter) {
    ...
    if (leftElementIsClicked == false && rightElementIsClicked == false) {
      ...
      if (roundNumber === randomBonusRound) {
        ...
        setTimeout(bonusRound, 3000); //go to bonus round after 3 seconds
      } else {
        setTimeout(updateTweets, 3000); //update Tweets after 3 seconds
      }
    }
  } else {
